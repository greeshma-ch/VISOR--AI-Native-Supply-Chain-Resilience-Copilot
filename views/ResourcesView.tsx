...
const ResourcesView: React.FC<ResourcesViewProps> = ({ onBack, context }) => {
  ...
  const recentResources: Resource[] = useMemo(() => {
    const base = [
      ...
      { id: 3, title: 'ChainGuard v3.0 Operational Manual', type: 'Manual', date: 'Jan 2026', category: 'handbooks', location: 'System' },
  ...
  const categories = useMemo(() => {
    const counts = recentResources.reduce((acc, res) => {
      const isArchive = res.title.includes('ARCHIVE') || res.title.includes('CASE STUDY');
      const catId = isArchive ? 'archives' : res.category;
...
${separator}
GENERATED BY CHAINGUARD AI CORE v2.5.4
              ...
              <div className="relative z-10">
                <div className="border-b-4 border-slate-900 pb-8 mb-12 flex flex-col sm:flex-row justify-between items-start sm:items-end gap-4">
                  ...
                  <div className="text-left sm:text-right">
                    ...
                    <p className="text-sm font-bold">CG-RES-{viewingResource.id}-{viewingResource.date.match(/\d{4}/)?.[0] || '2026'}</p>
                  </div>
                </div>
                ...
                <div className="space-y-16">
                  ...
                  <div className="mt-32 pt-12 border-t-2 border-slate-200 flex flex-col sm:flex-row justify-between items-center gap-8 opacity-60">
                    ...
                    <div className="text-center sm:text-right">
                      ...
                      <p className="text-[10px] font-medium uppercase tracking-widest">Generated by ChainGuard AI Core v2.5.4 // {new Date().toLocaleDateString()}</p>
                    </div>
                  </div>
                </div>
              </div>
